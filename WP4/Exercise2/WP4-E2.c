...
/*------------------------------------------------------------
EXERCISE 2
The system implements an analog timer (seconds only), using interrupts on timer.
For this it uses a motor module to move (Micro Servo in TinkerCad).
...
void callback()     //using interrupts
{
  movePulse(rotation);
  rotation++; //increment rotation
  seconds++; //increment timer
  Serial.print("Seconds: "); //print a context message
  Serial.println(seconds); //print seconds
}

// One empirical solution to avoid the problem of overlapping libraries (timer and servo),
//was to find a way to calculate the servo motor positon manually, have found this:
//https://stackoverflow.com/questions/49056510/arduino-servo-without-library

void movePulse(int x){ //pulse the motor
    int del=(7*x)+500; // calculating the values empirically for servo-pulse 500 to 1260us
    for (int pulseCounter=0; pulseCounter<=50; pulseCounter++){ //set a counter
